  ...
  /**
   * Nota: useremo solo puntatori a Contact, e chiameremo per semplicità
   * "contatto" un puntatore a Contact
   */
   ...
   * @return Il puntatore alla struttura che rappresenta il contatto, NULL se
   * ((name == NULL) || (surname == NULL))
   */
  ContactPtr mkContact(char *name, char *surname, char *mobile, char *url);

  /**
   * @brief Dealloca una struttura Contact e mette il relativo puntatore (che
   * dunque deve essere passato per riferimento) a NULL
   *
   * @param cntptr il puntatore al contatto (non può essere NULL). Nota: cntptr è
   * un puntatore a puntatore a Contact
   */
   ...
   * @param mobile La stringa che contiene il nuovo numero di cellulare  (può
   * essere NULL)
   */
   ...
   * @return 1 se i due contatti hanno lo stesso cognome e lo stesso nome, 0
   * altrimenti
   */
  _Bool equalsContact(const ContactPtr cnt1, const ContactPtr cnt2);

  /**
   * @brief Confronta due contatti in ordine lessicografico in base al cognome e
   * al nome
   *
   * @param cnt1 Il primo contatto
   * @param cnt2 Il secondo contatto
   * @return -1 se cnt1 minore cnt2, 0 se cnt1 uguale cnt2, 1 se cnt1 maggiore
   * cnt2
   */
